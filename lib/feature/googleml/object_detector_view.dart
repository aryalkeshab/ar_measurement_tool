import 'package:ar_measurement_tool/feature/googleml/camera_view.dart';
import 'package:ar_measurement_tool/feature/googleml/object_detector_painter.dart';
...
class _ObjectDetectorView extends State<ObjectDetectorView> {
  ...
  final _options = {
    'default': '',
    'all_object': 'yolov8n_int8.tflite',
  ...
  Future<void> _processImage(InputImage inputImage) async {
    ...
    if (inputImage.metadata?.size != null &&
      ...
      for (final object in objects) {
        ...
        text +=
            'Object: trackingId: ${object.trackingId}\nLabels: ${object.labels.map((e) => e.text).join(", ")}\n'
            'BoundingBox: x: ${boundingBox.left}, y: ${boundingBox.top}, width: ${boundingBox.width}, height: ${boundingBox.height}\n\n';
      }
      _text = text;
      ...
      for (final object in objects) {
        ...
        print('  x: ${boundingBox.left}');
        print('  y: ${boundingBox.top}');
        print('  width: ${boundingBox.width}');
        print('  height: ${boundingBox.height}');
      }

      _customPaint = null; // Optional: reset custom paint
    }
    _isBusy = false;
    if (mounted) {
      setState(() {});
